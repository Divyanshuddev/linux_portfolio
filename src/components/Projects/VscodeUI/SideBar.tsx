...
import { selectMenuBar } from "../../../features/ProjectSlice/ProjectSlice";
            ...
            <Stack spacing={1}>
                {
                    data.slice(0, 5).map((value, index) => {
                        return (
                            <Stack component={'button'} key={index} onClick={() => dispatch(selectMenuBar(value.title))} sx={selectedMenuBar === value.title ? styles.selectedIconButton : styles.iconButton} >
                                {value.icon}
                            </Stack>
                        )
                    })
